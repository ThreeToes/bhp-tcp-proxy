...
class ProxyServer:
    def __init__(self, target_host, target_port, server_addr, server_port):
        ...
        self.__target_post = target_port
    ...
    def __reader_loop(self):
        while not self.__close:
            (rs, ws, xs) = select.select([item for tempList in self.__socket_pairs for item in tempList], [], [], 0.1)
            for s in rs:
                matching_sock = None
                pair = None
                for (x, y) in self.__socket_pairs:
